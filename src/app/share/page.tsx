...
function buildSiteUrl(slug: string) {
  // If you already have wildcard subdomains wired: slug.capsuleweb.site
...
export default function SharePage() {
  ...
  useEffect(() => {
    // You’ll redirect here like:
    // /share?slug=pam&name=Pam&desc=AI%20coffee%20blog
  ...
  function openIGTip() {
    // No direct “post to Instagram” web flow.
    // This is a helpful UX: copy text + link and tell them to screenshot.
    copyLink()
      ...
      <div className="max-w-2xl w-full">
        ...
        <div className="rounded-[32px] border border-white/10 bg-gradient-to-b from-white/10 to-white/5 p-8 shadow-2xl">
          ...
          <p className="mt-6 text-xs text-white/50">
            Tip: this card is designed to be screenshot. (On Mac: ⇧⌘4)
          </p>
        </div>

        {/* Footer */}
        <div className="mt-6 text-center text-xs text-white/40">
          capsuleweb.site • built in minutes
        </div>
      </div>
    </main>
  )
